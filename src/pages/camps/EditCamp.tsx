...
import { useCamps, useDoctors, useStatesHierarchy } from '@/hooks/useApiData';
...
export default function EditCamp() {
  ...
  useEffect(() => {
    if (camp) {
      setFormData({
        ...
        stateId: camp.stateId || 0,
        state: camp.state || '',
        districtId: camp.districtId || 0,
        mandalId: camp.mandalId || 0,
        mandal: camp.mandal || '',
  ...
  const toggleDoctor = (doctorId: string) => {
    const ids = formData.doctorIds as string[];
    if (ids.includes(doctorId)) {
      updateField('doctorIds', ids.filter((i: string) => i !== doctorId));
    } else {
      updateField('doctorIds', [...ids, doctorId]);
    }
  };
          ...
          <Card className="flex-1 shadow-sm">
            ...
            <CardContent className="p-4 space-y-4">
              <div className="space-y-1">
                ...
                <Input id="name" placeholder="Enter camp name" value={formData.name} onChange={(e) => updateField('name', e.target.value)} className={`h-9 text-sm ${errors.name ? 'border-destructive' : ''}`} />
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="space-y-1">
                  ...
                  <Select
                    ...
                    onValueChange={(v) => {
                      ...
                      updateField('stateId', stateObj?.id || 0);
                      updateField('state', stateObj?.name || '');
                      updateField('districtId', 0);
                      updateField('district', '');
                      updateField('mandalId', 0);
                      updateField('mandal', '');
                    }}
                  >
                ...
                <div className="space-y-1">
                  <Label className="text-xs font-medium">District <span className="text-destructive">*</span></Label>
                  <Select
                    ...
                    onValueChange={(v) => {
                      ...
                      updateField('districtId', districtObj?.id || 0);
                      updateField('district', districtObj?.name || '');
                      updateField('mandalId', 0);
                      updateField('mandal', '');
                    }}
                  ...
                  >
                    ...
                    <SelectContent>
                      {statesHierarchy.find(s => s.id === formData.stateId)?.districts.map((district) => (
                ...
                <div className="space-y-1">
                  <Label className="text-xs font-medium">Mandal <span className="text-destructive">*</span></Label>
                  <Select
                    ...
                    onValueChange={(v) => {
                      ...
                      updateField('mandalId', mandalObj?.id || 0);
                      updateField('mandal', mandalObj?.name || '');
                    }}
                    disabled={!formData.districtId}
                  >
                    <SelectTrigger><SelectValue placeholder="Select mandal" /></SelectTrigger>
                    <SelectContent>
                      {statesHierarchy.find(s => s.id === formData.stateId)?.districts
                        .find(d => d.id === formData.districtId)?.mandals.map((mandal) => (
                          <SelectItem key={mandal.id} value={String(mandal.id)}>{mandal.name}</SelectItem>
                        )) || []}
                    </SelectContent>
                  </Select>
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1"><Label htmlFor="startDate" className="text-xs font-medium">Start Date <span className="text-destructive">*</span></Label><Input id="startDate" type="date" value={formData.startDate} onChange={(e) => updateField('startDate', e.target.value)} className={`h-9 text-sm ${errors.startDate ? 'border-destructive' : ''}`} />{errors.startDate && <p className="text-xs text-destructive">{errors.startDate}</p>}</div>
                <div className="space-y-1"><Label htmlFor="endDate" className="text-xs font-medium">End Date <span className="text-destructive">*</span></Label><Input id="endDate" type="date" value={formData.endDate} onChange={(e) => updateField('endDate', e.target.value)} className={`h-9 text-sm ${errors.endDate ? 'border-destructive' : ''}`} />{errors.endDate && <p className="text-xs text-destructive">{errors.endDate}</p>}</div>
              </div>
              <div className="space-y-1"><Label htmlFor="description" className="text-xs font-medium">Description</Label><Textarea id="description" placeholder="Enter camp description" value={formData.description} onChange={(e) => updateField('description', e.target.value)} rows={2} className="text-sm" /></div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-4">
              <div className="space-y-2">
                {statusOptions.map((option) => (
                  <button key={option.value} onClick={() => updateField('status', option.value)} className={`w-full px-3 py-2 rounded-lg border text-sm font-medium transition-all ${formData.status === option.value ? `${option.color} ring-2 ring-offset-1 ring-accent` : 'bg-white border-border hover:bg-muted'}`}>{option.label}</button>
                ))}
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-4">
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {allDoctors.map((doctor) => (
                  <div key={doctor.id} className={`flex items-center justify-between p-2 rounded-lg cursor-pointer transition-colors ${(formData.doctorIds as string[]).includes(String(doctor.id)) ? 'bg-accent/20 border border-accent' : 'hover:bg-muted border border-transparent'}`} onClick={() => toggleDoctor(String(doctor.id))}>
                    <div><p className="font-medium text-sm">{doctor.name}</p><p className="text-xs text-muted-foreground">{doctor.specialization}</p></div>
                    {(formData.doctorIds as string[]).includes(String(doctor.id)) && <Badge className="bg-accent text-accent-foreground text-xs">✓</Badge>}
                  </div>
                ))}
              </div>
              {(formData.doctorIds as string[]).length > 0 && <div className="mt-3 pt-3 border-t"><p className="text-xs text-muted-foreground">{(formData.doctorIds as string[]).length} doctor(s) assigned</p></div>}
            </CardContent>
          </Card>
        </div>
      </div>
